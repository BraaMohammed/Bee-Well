...
const test =  [
    ...
    //JSON.parse(intialContentFocused)
...
export default function App({ getEditorState ,  intialContentFocused ,  setCurrentHtmlNoteContent , isNewNote , setHtmlContentForNewNotes }) {
    ...
    const handleEditorChange = async (editor) => {
        ...
        const filteredContent = content.filter(block => 
          (block.content && block.content.length > 0) || block.type === "image" || block.type === "video" || block.type === "file" || block.type === "audio"
        );
    ...
    return (
        ...
        <BlockNoteView  theme={redTheme} editor={editor} formattingToolbar={false} onChange={handleEditorChange}
             test-css              >
